...
启动登录AI智能量化软件 Station后，点击【交易】按钮，在配置对话框中的【应用模块】栏勾选【PaperAccount】。
...
在启动模块之前，请先连接要进行模拟交易的接口（连接方法详见基本使用篇的连接接口部分）。看到AI智能量化软件 Trader主界面【日志】栏输出“合约信息查询成功”之后再启动模块，如下图所示：
...
- 数据的持久化保存：
  ...
  - 成交数据和委托数据不保存，关闭AI智能量化软件 Trader后即消失；
  - 持仓数据会在有变化时**立即写入硬盘文件**，重启AI智能量化软件 Trader登录交易接口后即可看到（要收到相应的合约信息）。
