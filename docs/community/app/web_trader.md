...
WebTrader是用于**Web应用后端服务**的功能模块，用户可以通过浏览器（而非PyQt桌面端）来运行管理AI智能量化软件量化策略交易。
...
后端服务包括两个独立的进程：
- 策略交易进程
  - 运行AI智能量化软件 Trader的进程，负责所有策略交易功能的运行；
...
启动登录AI智能量化软件 Station后，点击【交易】按钮，在配置对话框中的【应用模块】栏勾选【WebTrader】。
