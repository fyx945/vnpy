...
对于数据服务，AI智能量化软件提供了标准化的接口BaseDatafeed（位于vnpy.trader.datafeed中），实现了更加灵活的数据服务支持。在全局配置中，和数据服务相关的字段都以datafeed作为前缀。
...
以上字段对于所有数据服务都是必填的，如果是token方式授权请填写在datafeed.password字段中。目前AI智能量化软件 Trader支持以下七种数据服务，**具体每个数据服务的细节可在对应的项目地址中找到**。
...
请注意，因为Tinysoft目前不支持Python3.10，所以AI智能量化软件 Studio 3.0.0没有提供Tinysoft支持。
