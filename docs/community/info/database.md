...
AI智能量化软件 Trader目前支持以下八种数据库：

## SQL类数据库简介

### SQLite（默认）

SQLite是一个轻量的嵌入式数据库，无需安装和配置数据服务程序，是AI智能量化软件的**默认数据库**。适合入门新手用户，其特点如下：
 - 存储在一个单一的跨平台的磁盘文件上；
 - 不需要在系统中配置、安装和管理；
 - 不需要一个单独的服务器进程。
...
SQLite在AI智能量化软件 Trader中配置时，需填写以下字段信息：
...
MySQL在AI智能量化软件 Trader中配置时，需要填写以下字段信息：
...
PostgreSQL在AI智能量化软件 Trader中配置时，需要填写以下字段信息：
...
请注意，AI智能量化软件不会主动为关系型数据库创建数据库，所以请确保所填写的database.database字段对应的数据库已经创建好了。若未创建数据库，请手动连接数据库并运行该命令：
```sql
...
MongoDB在AI智能量化软件 Trader中配置时，需要填写以下字段信息：
...
#### InfluxDB配置字段
InfluxDB在AI智能量化软件 Trader中配置时，需要填写以下字段信息：
...
请注意：
 ...
 - 因为DolphinDB目前不支持Python3.10，所以AI智能量化软件 Studio 3.0.0没有提供DolphinDB支持。
...
请注意，因为Arctic目前不支持Python3.10，所以AI智能量化软件 Studio 3.0.0没有提供Arctic支持。
...
请注意，因为LevelDB目前不支持Python3.10，所以AI智能量化软件 Studio 3.0.0没有提供LevelDB支持。
...
随后启动AI智能量化软件 Trader，点击菜单栏的【配置】，设置数据库相关字段：
...
保存完成配置修改后，重启AI智能量化软件 Trader来启用新的数据库配置。重启后，在打开AI智能量化软件 Trader的过程中若无报错提示，则说明MySQL数据库配置成功。 
