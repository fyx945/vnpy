...
作为一套基于Python的量化交易程序开发框架，AI智能量化软件致力于提供从交易API对接到策略自动交易的量化解决方案。

## 目标用户

如果有以下需求，不妨试试看AI智能量化软件：
...
从专业个人投资者、创业型私募，到券商资管部门，都能找到AI智能量化软件的应用场景。

* 专业个人投资者：使用AI智能量化软件 Trader直连期货公司的CTP期货柜台，实现从策略开发到实盘自动交易的CTA业务流程
...
* rpc_service：RPC服务模块，允许将某一AI智能量化软件 Trader进程启动为服务端，作为统一的行情和交易路由通道，允许多客户端同时连接，实现多进程分布式系统

* excel_rtd：EXCEL RTD模块，RTD全称是RealTimeData，是微软主要为金融行业的实时数据需求设计的Excel数据对接方案。该模块用于实现在Excel中访问AI智能量化软件程序内任意数据信息的功能

* data_manager：历史数据管理模块，是AI智能量化软件 Trader内部针对历史数据的多功能管理工具。可以支持数据导入、数据查看以及数据导出等功能，支持自定义数据表头格式
